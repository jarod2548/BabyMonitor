...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<String> handleConflict(DataIntegrityViolationException e) {
    logger.error("Error", e);
    return ResponseEntity.status(HttpStatus.CONFLICT).body("Database constraint error");
  }
  ...
  public ResponseEntity<String> handleAuthorize(InvalidCredentialsException e) {
    logger.error("Error", e);
    return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Geen toegang");
  }
  ...
  public ResponseEntity<String> handleUnexpected(Exception e) {
    logger.error("Error", e);
    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Onverwachte error");
  }
}
